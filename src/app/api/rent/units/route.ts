...
export async function PATCH(req: Request) {
  try {
    ...
    const updated = await db.update(rentUnits).set(rest).where(rentUnits.id.eq(id)).returning();
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "Missing id" }, { status: 400 });
    const deleted = await db.delete(rentUnits).where(rentUnits.id.eq(id)).returning();
