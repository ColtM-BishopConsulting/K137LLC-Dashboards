...
export async function PATCH(req: Request) {
  try {
    ...
    const updated = await db.update(timeEntries).set(rest).where(timeEntries.id.eq(id)).returning();
...
export async function DELETE(req: Request) {
  try {
    ...
    if (!id) return NextResponse.json({ error: "Missing id" }, { status: 400 });
    const deleted = await db.delete(timeEntries).where(timeEntries.id.eq(id)).returning();
